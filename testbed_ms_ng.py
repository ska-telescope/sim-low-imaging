...
python testbed_ms.py --msname data/EoR0_20deg_24.MS --weighting natural --context 2d --fov 0.05
...
from processing_components.imaging.base import invert_ng
...
if __name__ == "__main__":
    ...
    parser.add_argument('--channels', type=int, nargs=2, default=[131,146], help='Channels to process')
    ...
    msname = args.msname
    bvis = create_blockvisibility_from_ms(msname, start_chan=ochannels[0], end_chan=ochannels[1])[-1]
    ...
    channel_bandwidth = numpy.array([numpy.sum(vis.channel_bandwidth)])
    ####################################################################################################################
    ...
    dirty, sumwt = invert_ng(vis.blockvis, polmodel, nthreads=threads)
    print(qa_image(dirty))
    print("Invert_ng finished in %.2f (seconds)" % (time.time() - start_epoch))
    
    show_image(dirty)
    plot_name = target_ms.split('/')[-1].replace('.MS', '_dirty_ng.jpg')
    plt.savefig(plot_name)
    plt.show(block=False)
    
    dirty_name = target_ms.split('/')[-1].replace('.MS', '_dirty_ng.fits')
    export_image_to_fits(dirty, dirty_name)

#    polmodel.data[0,0,npixel//2,npixel//2] = 1.0
#    vis = predict_ng(vis, polmodel)
